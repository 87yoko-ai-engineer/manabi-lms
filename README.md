...
| **Tailwind不使用(素のCSS+デザイントークン)** | デザインプロトタイプ(Claude Design)から移植したクラス名・トークンをそのまま活かすため。配色・余白はCSS変数で一元管理 |
...
| 対策 | 内容 |
|---|---|
| 認可の二重検証 | ミドルウェア(proxy.ts)+ DAL / Server Actions の双方でサーバー側検証。クライアントのボタン制御に依存しない |
| パスワード | bcrypt ハッシュ化。管理者発行時の最小長は 12文字(NIST SP 800-63B 準拠) |
| アカウント列挙対策 | ログイン失敗時に「メール不存在」と「パスワード誤り」を区別しない(ERR-01) |
| セキュリティヘッダー | CSP(YouTube埋め込みのみ frame 許可)・X-Frame-Options・X-Content-Type-Options・Referrer-Policy・Permissions-Policy を `next.config.ts` で全ページに付与 |
| サーバー側入力検証 | カラーコード・動画ID の形式検証(CSS / iframe src への注入対策)、各フィールドの最大長、メール形式 |
| なりすましの安全性 | 管理者の受講者ビューは httpOnly Cookie + 管理者セッション時のみ参照(受講者が自分でセットしても無効) |

### 本番化する場合の追加課題(デモでは未実装)

- **ログインのレート制限** — 現状はブルートフォース対策が bcrypt の計算コストのみ。本番化する場合は Upstash Redis(Vercel Marketplace)等で「同一IP・同一メールごとの試行回数制限」を追加する。インメモリのカウンタはサーバーレス環境ではインスタンス間で共有されず効果がないため、外部ストアが必須
- **CSP の nonce 化** — 現状は Next.js のインラインスクリプト許容のため `script-src 'unsafe-inline'`。middleware で nonce を生成・付与する方式に移行するとより厳格になる
- **監査ログ** — 管理操作(講座削除・受講者削除等)の操作履歴テーブル
...
その他: 公開期間外・受講期間外講座のロック表示(ERR-07/08)。
