  ...
  return (
    <div>
      {course.chapters.map((ch, ci) => (
        <ChapterEditor key={ch.id} chapter={ch} index={ci} total={course.chapters.length} run={run} pending={pending} />
      ))}
      ...
      <div className="ed-add-chapter">
        ...
        <button className="btn-ghost" disabled={pending} onClick={() => run(async () => {
          const res = await createChapter(course.id, newChapter);
          if (res.ok) setNewChapter("");
          return res;
        })}>
          <Icons.plus size={15} />チャプター追加
        </button>
      </div>
...
function ChapterEditor({ chapter, index, total, run, pending }: {
  ...
  return (
    <div className="ed-chapter">
      <div className="ed-chapter-head">
        ...
        {title !== chapter.title && (
          <button className="btn-ghost" disabled={pending} onClick={() => run(() => updateChapter(chapter.id, title))}>保存</button>
        )}
      ...
      <div className="ed-units">
        {chapter.units.map((u, ui) => (
          <UnitEditor key={u.id} unit={u} index={ui} total={chapter.units.length} run={run} pending={pending} />
        ))}
...
function UnitEditor({ unit, index, total, run, pending }: {
  ...
  return (
    <div className="ed-unit">
      ...
      <div className="ed-row-btns">
        {dirty && (
          <button className="ed-ibtn" title="保存" disabled={pending} style={{ color: "var(--brand)" }}
